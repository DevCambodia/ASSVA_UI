...
app.controller('mainCtrl', function($scope, $http) {
	...
	$scope.loadItem = function() {
		$http.get(
				BAES_URL + '/api/item/user?userId=' + USER_SESSION.ID
						+ '&page=' + $scope.page + '&limit=' + $scope.limit
						+ "&itemName=" + $scope.itemName).then(function(res) {
	...
	// document.getElementById('proImages').onchange = function(){
	//
	// var frmData = new FormData();
	// var files = $('#proImages')[0].files;
	// for(var i=0; i<files.length; i++){
	// frmData.append('file', files[i]);
	// }
	// uploadMultiple(frmData);
	// };
	//	
	// function uploadMultiple(frmData){
	//		
	// $http({
	// url : BAES_URL + '/api/upload/multiple',
	// method: 'POST',
	// data: frmData,
